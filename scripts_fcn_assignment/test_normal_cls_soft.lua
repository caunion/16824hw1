...
opt.network = '/media/data/daoyuan/16824/models/fcn_3.net' -- your trained network
opt.list_file = '/media/data/daoyuan/16824/3d/list/testLabels.txt'
opt.path_dataset = '/media/data/daoyuan/16824/3d/croptest/'
opt.codebooktxt = '/media/data/daoyuan/16824/3d/list/codebook_40.txt'
opt.resultpath =  '/media/data/daoyuan/16824/hw1/result' -- save your results here
if onserver then
    opt.network = '/home/daoyuanj/models/fcn_2.net' -- your trained network
    ...
    opt.resultpath =  '/home/daoyuanj/result' -- save your results here
...
function getSamples( N, beg)
  ...
  for n = 1,N do
    ...
    if not paths.filep(path_dataset .. filename) then
        print(string.format('warning, file not exists, jump. %s', (path_dataset .. filename)))
        jump = true
    end
    if not jump then
  ...
  simplenorm(imgs)
    ...
    imgs = imgs:index(2, perm)

    local samples = model_FCN:forward(imgs)
    samples = samples:view(N, 16, 16, 40):transpose(2,4):transpose(3,4)
--    print('samples shape')
--    print(#samples)
